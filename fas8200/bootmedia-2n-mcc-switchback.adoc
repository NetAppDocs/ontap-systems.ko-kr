...
summary: 2노드 MetroCluster 구성에서 부팅 미디어 교체를 완료한 후 FAS8200 시스템에서 MetroCluster 스위치백 작업을 수행하십시오. FAS8200 시스템은 수동 부팅 미디어 복구 절차만 지원합니다. 자동 부팅 미디어 복구는 지원되지 않습니다. 
---
...
[role="lead"]
2노드 MetroCluster 구성에서 부팅 미디어 교체를 완료한 후 FAS8200 시스템에서 MetroCluster 스위치백 작업을 수행하십시오. 이 절차에는 노드 및 스토리지 가상 머신(SVM) 상태 확인, 스위치백 명령 실행, 그리고 동기화 소스 SVM이 로컬 디스크 풀에서 데이터를 제공하는 정상 작동 상태로 구성이 복귀하는지 확인하는 작업이 포함됩니다.
