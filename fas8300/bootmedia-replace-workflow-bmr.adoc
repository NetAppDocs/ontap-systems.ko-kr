...
summary: FAS8300 또는 FAS8700 스토리지 시스템의 부트 미디어를 교체하려면 교체 요구 사항을 검토하고, 컨트롤러를 종료하고, 부트 미디어를 교체하고, 부트 미디어에 이미지를 복원하고, 시스템 기능을 확인하는 등의 작업을 수행하세요. 
---
= 부팅 미디어 자동 복구 워크플로 - FAS8300 및 FAS8700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
부트 이미지 자동 복구는 시스템이 적절한 부트 메뉴 옵션을 자동으로 식별하고 선택하는 과정을 포함합니다. 파트너 노드의 부트 이미지를 사용하여 FAS8300 또는 FAS8700 스토리지 시스템의 교체 부트 미디어에 ONTAP 다시 설치합니다.
