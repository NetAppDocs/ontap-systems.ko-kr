...
summary: 클러스터 및 HA I/O 모듈은 클러스터링 및 고가용성을 위한 상호 연결을 지원합니다. 모듈에 장애가 발생하고 스토리지 시스템이 특정 요구 사항을 충족하는 경우, ASA A20, ASA A30 또는 ASA A50 스토리지 시스템에서 모듈을 핫스왑할 수 있습니다. 
---
