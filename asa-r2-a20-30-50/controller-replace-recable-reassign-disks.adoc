...
summary: 'ASA A20, ASA A30 또는 ASA A50 스토리지 시스템이 정상 작동을 재개할 수 있도록 스토리지 리소스의 제어권을 교체 컨트롤러로 반환하세요.  반환 절차는 시스템에서 사용하는 암호화 유형(암호화 안 함, 온보드 키 관리자(OKM) 암호화, 외부 키 관리자(EKM) 암호화)에 따라 다릅니다.' 
---
= ASA A20, ASA A30 및 ASA A50 컨트롤러를 반납합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA A20, ASA A30 또는 ASA A50 스토리지 시스템이 정상 작동을 재개할 수 있도록 스토리지 리소스의 제어권을 교체 컨트롤러로 반환하세요.  반환 절차는 시스템에서 사용하는 암호화 유형(암호화 없음, 온보드 키 관리자(OKM) 암호화, 외부 키 관리자(EKM) 암호화)에 따라 다릅니다.
