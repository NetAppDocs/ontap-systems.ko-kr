...
sidebar: sidebar 
keywords: aff a700s, aff, a700s, procedure, secondary, boot, media, restore, fail, image, tgz, file, transfer, usb, flash, drive, transfer the boot image to the boot media using a usb flash drive, manual, manual boot recovery 
summary: 보조 부팅 미디어 또는 USB 플래시 드라이브를 사용하여 AFF A700s 시스템의 교체 부팅 미디어에 부팅 이미지를 전송하십시오. AFF A700s 시스템은 수동 부팅 미디어 복구 절차만 지원합니다. 자동 부팅 미디어 복구는 지원되지 않습니다. 
---
= USB 플래시 드라이브를 사용하여 AFF A700s 시스템의 부팅 미디어로 부팅 이미지 전송
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A700s 시스템에서 보조 부팅 미디어(기본 방법) 또는 USB 플래시 드라이브(보조 부팅 미디어 복원이 실패하거나  `image.tgz` 파일이 없는 경우)를 사용하여 부팅 이미지를 교체 부팅 미디어로 전송합니다.

AFF A700s 시스템은 수동 부팅 미디어 복구 절차만 지원합니다. 자동 부팅 미디어 복구는 지원되지 않습니다.



[[option-1-transfer-the-system-image-using-backup-recovery-from-the-second-boot-media]]
== 옵션 1: 두 번째 부팅 미디어의 백업 복구를 사용하여 시스템 이미지를 전송합니다

컨트롤러 모듈에 있는 두 번째 부팅 미디어의 이미지를 사용하여 교체용 부팅 미디어에 시스템 이미지를 설치할 수 있습니다. 이는 컨트롤러 모듈에 부팅 미디어가 두 개 있는 시스템의 주요 설치 방법입니다.

보조 부팅 미디어의 이미지에는  `image.tgz` 파일이 포함되어 있어야 하며 오류가 발생하지 않아야 합니다.  `image.tgz` 파일이 없거나 부팅 미디어에서 오류가 발생하는 경우 이 절차를 사용할 수 없습니다. 대신 <<옵션 2: USB 플래시 드라이브를 사용하여 부팅 이미지를 전송합니다.>>을(를) 사용하십시오.
...
[[option-2-transfer-the-boot-image-using-a-usb-flash-drive]]
== 옵션 2: USB 플래시 드라이브를 사용하여 부팅 이미지를 전송합니다.

이 절차는 보조 부팅 미디어 복원이 실패했거나 보조 부팅 미디어에서  `image.tgz` 파일을 찾을 수 없는 경우에만 사용해야 합니다.

* FAT32로 포맷된 USB 플래시 드라이브가 있어야 하며 4GB 이상의 용량이 있어야 합니다.
* 손상된 컨트롤러에서 실행 중이던 것과 동일한 ONTAP 이미지 버전의 사본. NetApp Support Site의 다운로드 섹션에서 적절한 이미지를 다운로드할 수 있습니다.
+
...
..  `savenv` 명령을 사용하여 변경한 환경 변수를 저장합니다.
