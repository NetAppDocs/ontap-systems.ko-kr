...
sidebar: sidebar 
keywords: aff a700s, aff, a700s, replace, real-time, clock, rtc, battery, controller, system, service, application, synchronization, function, impair, module, reinstall, replacement, complete, process, replace the real-time clock battery, shut down the impaired controller, remove the controller module, replace the rtc battery, reinstall the controller module and setting time and date after rtc battery replacement, complete the replacement process 
summary: 정확한 시간 동기화에 의존하는 서비스 및 애플리케이션이 계속 작동하도록 AFF A700s 컨트롤러 모듈의 실시간 클록 배터리를 교체하십시오. 
---
= AFF A700s 시스템의 실시간 클록 배터리를 교체하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
정확한 시간 동기화에 의존하는 서비스 및 애플리케이션이 계속 작동하도록 AFF A700s 시스템의 실시간 클록(RTC) 배터리를 교체하십시오.
...
+
image::../media/drw_a700s_open_air_duct.png[에어 덕트를 엽니다]
...
. RTC 배터리를 찾습니다.
+
image::../media/drw_a700s_rtc_battery_replace.png[RTC 배터리를 교체합니다]
...
[[step-4-reinstall-the-controller-module-and-setting-timedate-after-rtc-battery-replacement]]
