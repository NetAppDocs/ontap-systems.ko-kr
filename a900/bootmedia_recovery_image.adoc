...
.. 환경 변수가 예상대로 설정되어 있지 않으면 setenvenvenEnvironment_variable_name changed_value 명령을 사용하여 환경 변수를 수정합니다.
...
. 콘솔 케이블을 복구된 노드로 이동하고 version -v 명령을 실행하여 ONTAP 버전을 확인합니다.
