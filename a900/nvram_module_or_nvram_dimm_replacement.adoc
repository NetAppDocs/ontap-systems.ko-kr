...
.단계
. 교체 컨트롤러가 유지보수 모드("*>" 프롬프트 표시)인 경우 유지보수 모드를 종료하고 로더 프롬프트로 이동합니다
. 교체 컨트롤러의 LOADER 프롬프트에서 컨트롤러를 부팅하고 시스템 ID 불일치로 인해 시스템 ID를 재정의하라는 메시지가 표시되면 y를 입력합니다.
. 교체 컨트롤러 콘솔에 반환 대기 중... 메시지가 표시된 후 정상적인 컨트롤러에서 새 파트너 시스템 ID가 자동으로 할당되었는지 확인합니다. '스토리지 페일오버 표시'
+
...
+
교체 컨트롤러에 속한 디스크에 새 시스템 ID가 표시되어야 합니다. 다음 예에서는 노드 1이 소유한 디스크에 새 시스템 ID 1873775277이 표시됩니다.
