...
summary: NVRAM 모듈은 NVRAM11 및 DIMM으로 구성되며, NVRAM 모듈당 최대 2개의 NVMe SSD Flash Cache 모듈을 포함할 수 있습니다. 
---
= AFF A900 시스템의 NVRAM 모듈 또는 NVRAM DIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A900 시스템에서 NVRAM 모듈이 고장 나거나 DIMM에 결함이 발생하면 NVRAM 모듈 또는 NVRAM DIMM을 교체하십시오. 교체 과정은 섀시에서 NVRAM 모듈을 분리하고, 고장 난 부품을 교체한 후, 모듈을 다시 설치하는 방식으로 진행됩니다.
...
+
image::../media/drw_a900_move-remove_NVRAM_module.svg[NVRAM 모듈을 분리합니다]
...
+
image::../media/drw_a900_remove_NVRAM_module_contents.svg[NVRAM 모듈 내용을 제거합니다]
