...
및 NVRAM DIMM을 교체하려면 NVRAMModule을 섀시에서 제거하고 모듈에 장애가 발생한 DIMM을 교체한 다음 NVRAM 모듈을 다시 설치해야 합니다.
...
+
https://netapp.hosted.panopto.com/Panopto/Pages/Viewer.aspx?id=6eb2d864-9d35-4a23-b6c2-adf9016b359f["애니메이션 -- NVRAM 모듈을 교체합니다"^]
...
.. 모듈 면의 측면에 있는 당김 탭을 당겨 섀시에서 NVRAM 모듈을 분리합니다.
+
https://netapp.hosted.panopto.com/Panopto/Pages/Viewer.aspx?id=0ae4e603-c22b-4930-8070-adf2000e38b5["애니메이션 -- NVRAM DIMM을 교체합니다"^]
...
== 5단계: 컨트롤러 모듈의 HA 상태를 확인하고 설정합니다
...
[role="lead"]
교체 노드를 부팅할 때 시스템 ID 변경을 확인한 다음 변경이 구현되었는지 확인해야 합니다.
...
. 교체 노드 콘솔에 반환 대기 중... 메시지가 표시된 후 정상 노드에서 새 파트너 시스템 ID가 자동으로 할당되었는지 확인합니다. '스토리지 페일오버 표시'
+
...
.. 모든 코어 덤프는 'system node run-node local-node-name 파트너 savecore'입니다
.. 반환 명령을 실행하기 전에 'avecore' 명령이 완료될 때까지 기다리십시오.
+
다음 명령을 입력하여 savecore 명령의 진행 상태를 모니터링할 수 있습니다. 'system node run-node local-node-name 파트너 savecore -s'
...
. 노드를 돌려줍니다.
+
.. 정상 노드에서 교체된 노드의 스토리지, 즉 'storage failover 반환 - ofnode replacement_node_name'을 반환하십시오
+
교체 노드가 스토리지를 다시 가져와 부팅을 완료합니다.
...
+
스토리지 failover show 명령의 출력에는 파트너 메시지에서 변경된 시스템 ID가 포함되어서는 안 됩니다.



. 디스크가 제대로 할당되었는지 확인합니다. '스토리지 디스크 표시-소유권'
+
교체 노드에 속한 디스크에 새 시스템 ID가 표시되어야 합니다. 다음 예에서는 노드 1이 소유한 디스크에 새 시스템 ID 1873775277이 표시됩니다.
...
+
MetroCluster 구성을 정상 상태로 되돌리려면 교체 후 몇 분 정도 걸리며, 이때 각 노드에 구성된 상태가 표시되며 DR 미러링이 활성화되고 정상 모드가 표시됩니다. MetroCluster node show-fields node-systemid 명령 출력에는 MetroCluster 구성이 정상 상태로 돌아갈 때까지 이전 시스템 ID가 표시됩니다.
...
** 교체 노드는 재해 사이트에 있는 디스크의 현재 소유자입니다.
+
