...
[role="lead"]
스토리지 암호화가 활성화된 경우 적절한 절차를 따르십시오.
...
. 프롬프트에 클러스터의 온보드 키 관리자 암호를 입력합니다. 예를 보려면 여기 를 참조하십시오. ******
. 백업 키 데이터를 입력합니다
+

NOTE: 암호 정보가 완료되면 노드가 부팅 메뉴로 돌아갑니다.
...
. 프롬프트에 관리 인증서 정보를 입력합니다. need example here *******
+
...
. 디스크의 MSID를 재설정합니다.
+
.. 온보드 저장소 암호화를 사용하는 경우 의 절차를 따릅니다 https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-onboard-task.html["FIPS 드라이브 또는 SED(온보드 키 관리)에 데이터 인증 키 할당"^]
.. 외부 키 관리자를 사용하고 있습니다. 의 절차를 따르십시오 https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-external-task.html["데이터 인증 키를 FIPS 드라이브 또는 SED(외부 키 관리)에 할당"^]
