...
sidebar: sidebar 
keywords: AFF A900 
summary: NVRAM11 배터리가 포함된 디스테이지 컨트롤러 전원 모듈을 핫 스왑하려면 고장난 DCPM 모듈을 찾아 섀시에서 분리한 다음 교체용 DCPM 모듈을 설치하십시오. 
---
= AFF A900 시스템의 NVRAM11 배터리가 포함된 DCPM 모듈을 교체하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A900 시스템의 디스테이지 컨트롤러 전원 모듈(DCPM)에 문제가 발생하면 핫스왑 방식으로 교체하십시오. DCPM에는 NVRAM11 배터리가 포함되어 있으며 데이터 보호 손실을 방지하기 위해 제거 후 5분 이내에 교체해야 합니다.
...
+
image::../media/drw_a900_remove_NV_battery.svg[NV 배터리를 탈거하십시오]
