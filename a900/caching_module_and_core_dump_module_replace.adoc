...
---
= 캐싱 모듈을 교체하거나 코어 덤프 모듈(FAS9500)을 추가/교체합니다
:icons: font
:imagesdir: ../media/
...
* 캐시 모듈의 핫 스와핑에 대한 지침은 을 참조하십시오 link:../a900/caching_module_hot_swap.html["캐시 모듈 핫 스왑"].
...
[role="lead"]
stdin>에서 확인되지 않은 지시문 - 포함::::./_include/shutdown_any_MCC_supported [ ]
...
... 코어 덤프 모듈 앞면에 있는 테라코타 분리 단추를 누릅니다.
+

IMPORTANT: 코어 덤프 모듈을 꺼낼 때 번호가 매겨진 입출력부 래치를 사용하지 마십시오. 번호가 매겨진 입출력부 래치는 코어 덤프 모듈이 아니라 전체 NVRAM10 모듈을 배출합니다.
...
== 2노드 MetroCluster 구성에서 애그리게이트를 다시 전환합니다
