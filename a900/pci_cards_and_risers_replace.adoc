...
. 시스템이 10GbE 클러스터 상호 연결 및 40GbE NIC의 데이터 연결을 지원하도록 구성된 경우 nicadmin 변환 명령을 사용하여 유지보수 모드에서 이러한 포트를 10GbE 연결로 변환합니다. 을 참조하십시오 https://["10GbE 연결을 위해 40GbE NIC 포트를 여러 10GbE 포트로 변환합니다"^] 를 참조하십시오.
+
