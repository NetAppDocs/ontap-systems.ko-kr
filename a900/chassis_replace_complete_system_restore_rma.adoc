...
. 에 설명된 대로 추가 상태 점검을 수행합니다 https://["How_to_Perform_a_cluster_health_check_with_a_script_in_ONTAP 을 참조하십시오"^]
...
* 모두 해결 https://["Active IQ Wellness 알림 및 위험"^] (Active IQ는 전원을 켠 후 자동 지원을 처리하는 데 시간이 걸립니다. 결과가 지연될 수 있습니다.)
* 실행 https://["Active IQ Config Advisor"^]
* 를 사용하여 시스템 상태를 확인합니다 https://["How_to_Perform_a_cluster_health_check_with_a_script_in_ONTAP 을 참조하십시오"^]
