...
summary: '2노드 MetroCluster 구성에서 FRU 교체를 완료한 후 MetroCluster 스위치백 작업을 수행할 수 있습니다. 이 작업을 통해 구성이 정상 작동 상태로 복원되며, 이전에 장애가 발생했던 사이트의 동기화 소스 스토리지 가상 머신(SVM)이 활성화되어 로컬 디스크 풀에서 데이터를 제공합니다.' 
---
= ASA A400 시스템의 2노드 MetroCluster 구성에서 애그리게이트를 다시 전환합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
