...
summary: AFF C80 시스템의 교체 부팅 매체에서 암호화를 복원하여 지속적인 데이터 보호를 보장합니다. 이 프로세스에는 키 가용성을 확인하고, 암호화 설정을 다시 적용하고, 데이터에 대한 보안 액세스를 확인하는 작업이 포함됩니다. 
---
= 부팅 미디어에서 암호화 복원 - AFF C80
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
