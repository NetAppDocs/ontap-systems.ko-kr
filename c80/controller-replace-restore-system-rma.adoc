...
summary: 논리 인터페이스가 홈 포트에 보고되는지 확인하고 클러스터 상태 점검을 수행한 다음 장애가 발생한 부품을 NetApp에 반환하여 AFF C80 컨트롤러 교체 워크플로우의 최종 단계를 완료합니다. 
---
= 전체 컨트롤러 교체 - AFF C80
:allow-uri-read: 
:icons: font
...
[role="lead"]
논리 인터페이스가 홈 포트에 보고되는지 확인하고 클러스터 상태 점검을 수행한 다음 장애가 발생한 부품을 NetApp에 반환하여 AFF C80 컨트롤러 교체 워크플로우의 최종 단계를 완료합니다.
