...
---
= ASA A250 시스템에서 컨트롤러 구성을 복원하고 확인합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
하드웨어 교체를 완료하고 ASA A250 시스템을 유지보수 모드로 부팅한 후에는 교체된 컨트롤러의 하위 수준 시스템 구성을 확인하고 필요에 따라 시스템 설정을 재구성해야 합니다.
...
.다음 단계
시스템 구성을 복원하고 확인한 후 link:controller-replace-recable-reassign-disks.html["시스템 케이블을 다시 연결하고 디스크를 재할당합니다"]을(를) 수행해야 합니다.
