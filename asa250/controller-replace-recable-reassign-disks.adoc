...
summary: ASA A250 시스템이 정상 작동을 재개할 수 있도록 스토리지 리소스의 제어를 교체 컨트롤러로 반환합니다. 
---
= ASA A250 시스템의 컨트롤러를 반납하여 정상 작동을 재개하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA A250 시스템 복원 및 시스템 구성 확인을 완료한 후에는 시스템 케이블을 다시 연결하고 디스크를 재할당해야 합니다.
...
.다음 단계
시스템 케이블을 다시 연결하고 디스크를 재할당한 후에는 link:controller-replace-restore-system-rma.html["컨트롤러 교체를 완료합니다"]을(를) 수행해야 합니다.
