...
summary: 시스템에서 플래시 수명이 거의 다 되었거나 식별된 NVDIMM이 정상이 아니라고 등록하면 AFF A320 컨트롤러 모듈의 NVDIMM을 교체하십시오. 결함이 있는 NVDIMM을 교체하지 않으면 시스템 패닉이 발생할 수 있습니다. 
---
= AFF A320 시스템의 NVDIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A320 시스템에서 플래시 메모리 수명이 거의 다 되었거나 NVDIMM에 이상이 감지되면 해당 NVDIMM을 교체하십시오. 불량 NVDIMM을 교체하지 않으면 시스템 오류가 발생할 수 있습니다.
...
image::../media/drw_a320_nvdimm_replace_animated_gif.png[NVDIMM을 교체합니다]
