...
summary: 시스템에서 수정 가능한 오류 정정 코드 또는 수정 불가능한 ECC 오류가 증가하는 경우 AFF A320 컨트롤러 모듈의 DIMM을 교체하십시오. 결함이 있는 DIMM을 교체하지 않으면 시스템 패닉이 발생할 수 있습니다. 
---
= AFF A320 시스템의 DIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A320 스토리지 시스템에서 과도한 수정 가능한 ECC 오류 또는 수정 불가능한 ECC 오류가 발생하는 경우(일반적으로 단일 DIMM 오류로 인해 발생), DIMM을 교체하십시오.
...
+
image::../media/drw_a320_dimm_map.png[DIMM 맵]
...
컨트롤러 모듈에서 구성 요소를 교체한 후 컨트롤러 모듈을 섀시에 재설치해야 합니다.
