---
permalink: a320/install-quick-guide.html 
sidebar: sidebar 
keywords: aff, a320, aff a320, guide, graphic, instruction, typical, installation, system, rack, cabling, cable, initial, system, bring-up, netapp, quick guide 
summary: 이 안내서는 랙 설치 및 케이블 연결에서부터 초기 시스템 시작까지 시스템을 설치하는 일반적인 방법에 대한 그래픽 지침을 제공합니다. NetApp 시스템 설치에 익숙한 경우 이 가이드를 사용하십시오. 
---
= 빠른 단계를 사용하여 AFF A320 스토리지 시스템 설치
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
AFF A320 스토리지 시스템 랙 설치 및 케이블 연결부터 초기 시스템 설정까지, 간편한 그림 설명서를 통해 설치할 수 있습니다.

설치 및 설정 지침_PDF 포스터에 액세스합니다.

link:../media/PDF/215-14058_A0_A320_ISI.pdf["AFF A320 설치 및 설정 지침"^]
