...
. system ID mismatch에 의한 시스템 ID 재정의를 묻는 메시지가 나타나면, _replacement_node의 LOADER 프롬프트에서 노드를 부팅하고 "y"를 입력합니다.
...
+
MetroCluster 구성을 정상 상태로 되돌리려면 교체 후 몇 분 정도 걸리며, 이때 각 노드에 구성된 상태가 표시되며 DR 미러링이 활성화되고 정상 모드가 표시됩니다. MetroCluster node_show -fields node -systemid_command 출력은 MetroCluster 구성이 정상 상태로 돌아갈 때까지 이전 시스템 ID를 표시합니다.
