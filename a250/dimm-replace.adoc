...
summary: 상태 모니터 경고를 기반으로 과도한 CECC 오류가 발생하거나 스토리지 시스템이 ONTAP를 부팅하지 못하게 하는 수정 불가능한 ECC 오류가 발생할 경우 AFF A250 컨트롤러의 DIMM을 교체하십시오. 
---
= AFF A250 시스템의 메모리 오류를 해결하려면 DIMM을 교체하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A250 시스템에서 스토리지 시스템에 과도한 수정 가능(CECC) 오류 또는 수정 불가능 메모리 오류가 발생하면 DIMM을 교체하십시오. 이러한 오류로 인해 스토리지 시스템이 ONTAP를 부팅하지 못할 수 있습니다.
