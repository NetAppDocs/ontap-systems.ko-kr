...
summary: 시스템에서 수정 가능한 오류 정정 코드 또는 수정 불가능한 ECC 오류가 증가하는 경우 AFF A800 컨트롤러 모듈의 DIMM을 교체하십시오. 결함이 있는 DIMM을 교체하지 않으면 시스템 패닉이 발생할 수 있습니다. 
---
= AFF A800 시스템의 메모리 오류를 해결하려면 DIMM을 교체하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A800 시스템에서 스토리지 시스템에 과도한 수정 가능(CECC) 오류 또는 수정 불가능 메모리 오류가 발생하면 DIMM을 교체하십시오. 이러한 오류로 인해 스토리지 시스템이 ONTAP를 부팅하지 못할 수 있습니다.
