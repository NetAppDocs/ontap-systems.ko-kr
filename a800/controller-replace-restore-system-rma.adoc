...
summary: AFF A800 시스템을 정상 작동 상태로 복원하려면 필요한 경우 NetApp Storage Encryption 구성을 복원하고 새 컨트롤러용 라이센스를 설치한 다음 고장난 부품을 NetApp에 반환하십시오. 
---
= AFF A800 시스템의 컨트롤러 교체를 완료합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A800 시스템의 컨트롤러 교체를 완료하려면 NetApp Storage Encryption 구성을 복원하고(필요한 경우), 논리 인터페이스(LIF)가 홈 포트로 보고하는지 확인하고, 클러스터 상태 점검을 수행한 다음, 고장난 부품을 NetApp에 반환하십시오.
