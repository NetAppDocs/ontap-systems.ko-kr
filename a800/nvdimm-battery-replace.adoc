...
---
= AFF A800 시스템의 NVDIMM 배터리를 교체합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A800 시스템의 NV 배터리는 정전 시 중요한 시스템 데이터를 보존하므로, 배터리 잔량이 부족해지거나 고장이 나면 교체하십시오. 배터리 교체 과정은 컨트롤러 모듈을 분리하고, 배터리를 교체한 후, 컨트롤러 모듈을 다시 설치하는 방식으로 진행됩니다.
