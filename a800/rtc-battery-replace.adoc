...
summary: 정확한 시간 동기화에 의존하는 서비스 및 애플리케이션이 계속 작동하도록 AFF A800 컨트롤러 모듈의 실시간 클록 배터리를 교체하십시오. 
---
= 실시간 시계 배터리 교체 - AFF A800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
정확한 시간 동기화에 의존하는 서비스 및 애플리케이션이 계속 작동하도록 하려면 AFF A800 시스템의 실시간 클록(RTC) 배터리(일반적으로 코인형 배터리라고 함)를 교체하십시오.
