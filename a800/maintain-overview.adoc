...
---
= AFF A800 스토리지 시스템 유지 관리에 대해 알아보십시오
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
장기적인 안정성과 최적의 성능을 보장하려면 AFF A800 스토리지 시스템의 하드웨어를 유지 관리하세요. 고장난 부품 교체와 같은 정기적인 유지 관리 작업을 수행하면 가동 중지 및 데이터 손실을 방지하는 데 도움이 됩니다.

유지 관리 절차에서는 AFF A800 스토리지 시스템이 이미 ONTAP 환경에서 스토리지 노드로 배포되었다고 가정합니다.
...
AFF A800 스토리지 시스템의 경우 다음 구성 요소에 대한 유지보수 절차를 수행할 수 있습니다.
