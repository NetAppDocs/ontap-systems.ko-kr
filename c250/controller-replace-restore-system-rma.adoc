...
summary: AFF C250 시스템의 컨트롤러 교체를 완료하여 시스템을 완전히 작동시키려면 NetApp 스토리지 암호화 구성을 복원하고 새 컨트롤러에 대한 라이선스를 설치한 다음 고장난 부품을 NetApp에 반환해야 합니다. 
---
= AFF C250 스토리지 시스템의 컨트롤러 교체를 완료합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C250 시스템의 컨트롤러 교체를 완료하여 시스템을 정상적으로 작동시키려면 NetApp Storage Encryption 구성을 복원하고(필요한 경우), 새 컨트롤러에 대한 라이센스를 설치한 다음, 키트에 동봉된 RMA 지침에 따라 고장난 부품을 NetApp으로 반환해야 합니다.
