...
[role="lead"]
<stdin>에서 해결되지 않은 지시문 - 포함::../_include/250_install-detailed-guide.adoc [ ]
...
.단계
. 25GbE 클러스터 인터커넥트 케이블을 사용하여 클러스터 인터커넥트 포트 e0c~e0c 및 e0d를 e0에 케이블로 연결합니다image:../media/oie_cable_sfp_gbe_copper.png[""]:
+
