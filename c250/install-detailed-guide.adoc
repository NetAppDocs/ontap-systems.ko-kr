...
---
= 자세한 가이드를 사용하여 AFF C250 스토리지 시스템을 설치하고 설정하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.단계
. 25GbE 클러스터 인터커넥트 케이블을 사용하여 클러스터 인터커넥트 포트 e0c를 e0c에, e0d를 e0d에 연결합니다.
+
...
NOTE: link:https://hwu.netapp.com["NetApp Hardware Universe를 참조하십시오"^] 호스트 네트워크 카드(파이버 채널 또는 25GbE)의 슬롯 우선 순위는 슬롯 2입니다. 하지만 두 카드를 모두 사용하는 경우 파이버 채널 카드는 슬롯 2에, 25GbE 카드는 슬롯 1에 장착합니다(아래 옵션 참조). 외장형 쉘프를 사용하는 경우 스토리지 카드는 쉘프에서 지원하는 유일한 슬롯인 슬롯 1에 장착합니다.
...
--
.옵션 2: 25GbE 데이터 또는 호스트 네트워크에 케이블 연결
--
컨트롤러의 25 GbE 포트는 25 GbE 데이터 또는 호스트 네트워크 스위치에 연결됩니다.
...
NOTE: 원격 관리를 위한 컨트롤러 콘솔 포트의 연속 연결을 설정하는 방법에 대한 자세한 내용은 기술 자료 문서를 참조하세요. https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/How_to_connect_an_ONTAP_controller_console_port_for_remote_management["원격 관리를 위해 ONTAP 컨트롤러 콘솔 포트를 연결하는 방법"^].
