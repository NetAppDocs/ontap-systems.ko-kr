...
summary: 시스템에서 플래시 수명이 거의 다 되었거나 식별된 NVDIMM이 전반적으로 정상 상태가 아니라고 등록되면 AFF C800 시스템에서 NVDIMM을 교체하십시오. 
---
= AFF C800 시스템의 NVDIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C800 시스템에서 플래시 메모리의 수명이 거의 다 되었거나 식별된 NVDIMM이 전반적으로 정상적이지 않다고 표시되면 NVDIMM을 교체하십시오. 그렇지 않으면 시스템 오류가 발생할 수 있습니다.

.시작하기 전에
* NetApp 에서 받은 교체용 NVDIMM이 AFF C800 시스템과 호환되는지 확인하십시오.
