...
---
= AFF C800 시스템의 케이블을 다시 연결하고 디스크를 재할당합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C800 시스템이 정상적으로 작동할 수 있도록 스토리지 리소스 제어권을 교체 컨트롤러로 반환하십시오.  반환 절차는 시스템에서 사용하는 암호화 유형(암호화 없음 또는 온보드 키 관리자(OKM) 암호화)에 따라 다릅니다.
