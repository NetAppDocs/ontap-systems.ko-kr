...
summary: NVRAM10 배터리가 포함된 디스테이지 컨트롤러 전원 모듈을 핫 스왑하려면 고장난 DCPM 모듈을 찾아 섀시에서 제거한 다음 교체용 DCPM 모듈을 설치하십시오. 
---
= AFF A700 시스템에서 디스테이지 컨트롤러 전원 모듈(DCPM)을 핫스왑 방식으로 교체합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A700 시스템에서 고장난 모듈을 서비스 중단 없이 교체하려면 디스테이지 컨트롤러 전원 모듈(DCPM)을 핫스왑 방식으로 교체하십시오.
...
+
image::../media/drw_9000_remove_nv_battery.png[DCPM 배터리를 제거합니다]
