...
---
= AFF C400 스토리지 시스템의 부팅 미디어에서 암호화 지원 확인
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
손상된 컨트롤러를 종료하기 전에 AFF C400 스토리지 시스템의 부팅 미디어에서 암호화 지원 여부를 확인하십시오. 여기에는 NetApp 볼륨 암호화(NVE)와의 ONTAP 버전 호환성을 확인하고 키 관리자 구성을 확인하는 작업이 포함됩니다.
