...
summary: '시스템에서 수정 가능한 오류 정정 코드(ECC) 수가 증가하는 경우 컨트롤러 모듈의 DIMM을 교체해야 합니다. 그렇지 않으면 시스템 패닉이 발생합니다.' 
---
= AFF C400 스토리지 시스템의 DIMM을 교체하여 메모리 오류를 해결합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C400 스토리지 시스템에서 과도한 수정 가능한 ECC 오류 또는 수정 불가능한 ECC 오류가 발생하는 경우, 일반적으로 단일 DIMM 오류로 인해 발생하는 이 문제를 해결하려면 해당 DIMM을 교체하십시오.
