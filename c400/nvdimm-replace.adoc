...
summary: 시스템에서 플래시 수명이 거의 다 되었거나 식별된 NVDIMM이 전반적으로 정상 상태가 아니라고 등록하면 컨트롤러 모듈의 NVDIMM을 교체해야 합니다. 그렇지 않으면 시스템 패닉이 발생합니다. 
---
= AFF C400 스토리지 시스템의 NVDIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C400 스토리지 시스템에서 수정 가능한 ECC 오류가 과도하게 발생하거나 수정 불가능한 ECC 오류가 발생하는 경우, 일반적으로 단일 NVDIMM 고장으로 인해 발생하는 이 문제를 해결하기 위해 해당 NVDIMM을 교체하십시오.
...
image::../media/drw_A400_Replace-NVDIMM-DIMM_IEOPS-1009.svg[NVDIMM을 교체합니다]
