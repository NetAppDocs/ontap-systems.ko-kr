...
---
= AFF C400 스토리지 시스템의 PCIe 또는 메자닌 카드 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C400 스토리지 시스템의 카드에서 케이블과 SFP/QSFP 모듈을 분리하고, 고장난 카드를 교체한 후 케이블을 다시 연결하여 PCIe 또는 메자닌 카드를 교체하십시오.
...
NOTE: 메자닌 카드 나사를 완전히 조이십시오. 나사가 헐거우면 e0e부터 e0h 포트가 감지되지 않거나 LED가 계속 깜빡일 수 있으며 로그에 "지원되지 않는 케이블" 오류가 표시될 수 있습니다.
