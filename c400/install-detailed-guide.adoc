...
---
= 자세한 가이드를 사용하여 AFF C400 스토리지 시스템을 설치하고 설정합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
자세한 가이드를 사용하여 AFF C400 스토리지 시스템을 설치하고 설정하십시오. 이 가이드는 랙 설치 및 케이블 연결부터 초기 시스템 가동까지 단계별 지침을 제공합니다.
...
image:../media/oie_cable_fiber_lc_connector.png["LC 파이버 커넥터"]
 a| 
메자닌 카드용 16Gb FC 또는 25GbE 케이블(주문에 따라 다름)
...
. 케이블 관리 장치를 컨트롤러 후면에 연결합니다(그림 참조).
+
image::../media/drw_c400_cable_tray_IEOPS-1208.svg[DRW C400 케이블 트레이 IEOPS 1208]
...
스위치가 없는 2노드 클러스터 방법을 사용하거나 전환된 클러스터 방법을 사용하여 컨트롤러를 네트워크에 연결할 수 있습니다.

.이 작업에 대해
* 카드의 포트 레이블이 보이지 않으면 카드 설치 방향(C400의 경우 PCIe 커넥터 소켓이 카드 슬롯의 왼쪽에 있음)을 확인하여 포트를 식별한 다음 NetApp Hardware Universe의 부품 번호로 카드를 찾습니다. 에서는 포트 레이블이 있는 베젤의 그림을 보여 줍니다. sysconfig -a 명령 또는 시스템 패킹 목록을 사용하여 카드 부품 번호를 찾을 수 있습니다.
* MetroCluster IP 구성을 케이블로 연결할 경우, 데이터 LIF 호스팅을 위해 포트 e0a/e0b를 사용할 수 있습니다(일반적으로 기본 IPSpace에 있음).
...
--
컨트롤러 모듈의 클러스터 인터커넥트 및 HA 포트는 파트너 컨트롤러 모듈에 케이블로 연결됩니다. 컨트롤러 모듈의 옵션 데이터 포트, 옵션 NIC 카드 및 관리 포트는 스위치에 연결됩니다.
...
.단계
. 그림을 사용하여 컨트롤러와 스위치 간의 케이블 연결을 완료합니다.
+
image::../media/drw_c400_TNSC-networking-cabling_IEOPS-1189.svg[DRW C400 TNSC 네트워킹 케이블 IEOPS 1189]
...
--
컨트롤러 모듈 클러스터 인터커넥트 및 HA 포트는 클러스터/HA 스위치에 케이블로 연결됩니다. 데이터 포트(옵션), NIC 카드(옵션), 메자닌 카드 및 관리 포트는 스위치에 연결됩니다.
...
. 그림을 사용하여 컨트롤러와 스위치 간의 케이블 연결을 완료합니다.
+
image::../media/drw_c400_switched_network_cabling_IEOPS-1190.svg[DRW C400 스위치 네트워크 케이블 IEOPS 1190]
...
.단계
. 다음 그림을 참조하여 컨트롤러를 단일 드라이브 쉘프에 연결합니다.
+
image::../media/drw_c400_one_ns224_shelf_IEOPS-1191.svg[drw C400 하나의 ns224 셸프 IEOPS 1191]
...
+
image::../media/drw_c400_two_ns224_shelves_IEOPS-1192.svg[DRW C400 2개의 ns224 셸프 IEOPS 1192]
