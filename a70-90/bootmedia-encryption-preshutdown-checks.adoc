...
summary: 'AFF A70 및 AFF A90 스토리지 시스템에서 데이터 보안을 보장하려면 암호화 키 지원 및 상태를 확인해야 합니다. ONTAP 버전이 NVE(NetApp 볼륨 암호화)를 지원하는지 확인하고 컨트롤러를 종료하기 전에 키 관리자가 활성 상태인지 확인합니다.' 
---
= 암호화 키 지원 및 상태 - AFF A70 및 AFF A90를 확인합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A70 및 AFF A90 스토리지 시스템에서 데이터 보안을 보장하려면 암호화 키 지원 및 상태를 확인해야 합니다. ONTAP 버전이 NVE(NetApp 볼륨 암호화)를 지원하는지 확인하고 컨트롤러를 종료하기 전에 키 관리자가 활성 상태인지 확인합니다.
