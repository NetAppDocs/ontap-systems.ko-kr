...
---
= AFF A70 또는 AFF A90 하드웨어의 네트워크 및 스토리지 연결을 위한 케이블 연결
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.스위치 클러스터 케이블링
--
100GbE 케이블을 사용하여 컨트롤러 A 및 B 포트 e1a와 e7a를 클러스터 네트워크 스위치 A 및 B에 연결합니다.
...
NOTE: 호스트 네트워크 연결에 포트 e1b 및 e7b를 사용하지 마십시오. 별도의 호스트 카드를 사용하십시오.
...
. 10/25 GbE 카드를 호스트 네트워크 스위치에 연결하십시오.
+
* 4포트, 10/25 GbE 호스트 *
...
.이 작업에 대해
AFF A70 및 AFF A90 스토리지 시스템은 NSM100 또는 NSM100B 모듈을 사용하는 NS224 쉘프를 지원합니다. 모듈 간의 주요 차이점은 다음과 같습니다.
...
.. 포트 e11b를 셸프 2의 NSM A 포트 e0b에 연결합니다.
+
...
====
.다음 단계
AFF A70 또는 AFF A90 시스템의 하드웨어 케이블을 연결한 후에는 link:install-power-hardware.html["AFF A70 또는 AFF A90 스토리지 시스템의 전원을 켭니다"].
