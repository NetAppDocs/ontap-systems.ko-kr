...
summary: 지나치게 수정할 수 있거나 수정할 수 없는 메모리 오류가 감지되면 AFF A70 또는 AFF A90 시스템의 DIMM을 교체합니다. 이러한 오류는 스토리지 시스템이 ONTAP를 부팅하지 못할 수 있습니다. 교체 프로세스에는 손상된 컨트롤러를 종료하고, DIMM을 교체하고, 컨트롤러를 다시 설치한 다음, 장애가 발생한 부품을 NetApp로 반환하는 과정이 포함됩니다. 
---
= AFF A70 또는 AFF A90 시스템의 DIMM을 교체하여 메모리 오류를 해결하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
지나치게 수정할 수 있거나 수정할 수 없는 메모리 오류가 감지되면 AFF A70 또는 AFF A90 시스템의 DIMM을 교체합니다. 이러한 오류는 스토리지 시스템이 ONTAP를 부팅하지 못할 수 있습니다. 교체 프로세스에는 손상된 컨트롤러를 종료하고, DIMM을 교체하고, 컨트롤러를 다시 설치한 다음, 장애가 발생한 부품을 NetApp로 반환하는 과정이 포함됩니다.
