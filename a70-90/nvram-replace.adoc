...
[role="lead"]
비휘발성 메모리에 결함이 있거나 업그레이드가 필요한 경우 AFF A70 또는 AFF A90 시스템의 NVRAM를 교체하십시오. 교체 프로세스에는 손상된 컨트롤러를 종료하고, NVRAM 모듈 또는 NVRAM DIMM을 교체하고, 디스크를 재할당하고, 장애가 발생한 부품을 NetApp에 반환하는 작업이 포함됩니다.

NVRAM 모듈은 NVRAM12 하드웨어와 현장 교체 가능한 DIMM으로 구성됩니다. 장애가 발생한 NVRAM 모듈 또는 NVRAM 모듈 내부의 DIMM을 교체할 수 있습니다.

.시작하기 전에
* 교체 부품이 있는지 확인합니다. 결함이 있는 구성 요소는 공급자로부터 받은 교체 구성 요소로 교체해야 합니다.
