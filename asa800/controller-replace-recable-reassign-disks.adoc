...
summary: ASA A800 시스템이 정상 작동을 재개할 수 있도록 스토리지 리소스의 제어를 교체 컨트롤러로 반환합니다. 
---
= ASA A800 시스템의 컨트롤러를 반납하여 정상 작동을 재개하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA A800 시스템이 정상적으로 작동할 수 있도록 스토리지 리소스 제어권을 교체 컨트롤러로 반환하십시오.  반환 절차는 시스템에서 사용하는 암호화 유형(암호화 없음 또는 온보드 키 관리자(OKM) 암호화)에 따라 다릅니다.
...
이 단계는 ONTAP HA 쌍으로 실행하는 시스템에만 적용됩니다.
