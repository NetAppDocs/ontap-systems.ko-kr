...
summary: AFX 2K 스토리지 시스템의 I/O 모듈에 오류가 발생하거나 성능 향상 또는 추가 기능 지원을 위해 업그레이드가 필요한 경우 해당 모듈을 교체하십시오. 교체 프로세스는 컨트롤러를 종료하고, 고장난 I/O 모듈을 교체하고, 컨트롤러를 재부팅한 후, 고장난 부품을 NetApp에 반환하는 과정으로 이루어집니다. 
---
= I/O 모듈 교체 - AFX 2K
:allow-uri-read: 
:icons: font
