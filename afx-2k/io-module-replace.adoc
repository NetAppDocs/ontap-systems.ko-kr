...
summary: AFX 2K 스토리지 시스템에서 모듈에 오류가 발생하거나 성능 향상 또는 추가 기능 지원을 위해 업그레이드가 필요한 경우 I/O 모듈을 교체하십시오. 교체 과정은 컨트롤러를 종료하고, 고장난 I/O 모듈을 교체하고, 컨트롤러를 재부팅한 후, 고장난 부품을 NetApp에 반환하는 단계로 구성됩니다. 
---
= AFX 2K 스토리지 시스템의 I/O 모듈 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX 2K 스토리지 시스템에서 I/O 모듈에 오류가 발생하면 해당 모듈을 교체하십시오. 교체 과정은 컨트롤러를 종료하고, 오류가 발생한 I/O 모듈을 교체하고, 컨트롤러를 재부팅한 후, 오류가 발생한 부품을 NetApp에 반환하는 것으로 구성됩니다.
