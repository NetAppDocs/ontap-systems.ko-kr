...
summary: AFX 2K 스토리지 시스템의 NVRAM12-EX는 비휘발성 메모리에 결함이 발생했거나 업그레이드가 필요한 경우 교체하십시오. 
---
= AFX 2K 스토리지 시스템의 NVRAM12-EX 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX 2K 스토리지 시스템의 비휘발성 메모리에 오류가 발생하거나 업그레이드가 필요한 경우 NVRAM12-EX를 교체하십시오. 교체 과정은 고장난 컨트롤러를 종료하고, NVRAM12-EX 모듈, NVRAM DIMM 또는 NVRAM 배터리를 교체한 후 고장난 부품을 NetApp에 반환하는 것을 포함합니다.
