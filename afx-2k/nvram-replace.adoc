...
summary: AFX 2K 스토리지 시스템의 비휘발성 메모리에 결함이 발생하거나 업그레이드가 필요한 경우 NVRAM을 교체하십시오. 
---
= AFX 2K 스토리지 시스템의 NVRAM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX 2K 스토리지 시스템의 비휘발성 메모리에 결함이 발생하거나 업그레이드가 필요한 경우 NVRAM을 교체하십시오. 교체 과정은 손상된 컨트롤러를 종료하고, NVRAM 모듈 또는 NVRAM DIMM을 교체한 후, 결함이 발생한 부품을 NetApp에 반환하는 단계로 구성됩니다.
...
+
image::../media/drw_afx_2k_nvram_remove_replace_ieops-2881.svg[NVRAM12 모듈 및 DIMM을 분리합니다]
...
+
image::../media/drw_afx_2k_nvram_remove_replace_ieops-2881.svg[NVRAM 12 모듈 및 DIMM을 분리합니다]
...
NOTE: 오버라이드 시스템 ID는 NVRAM 모듈 교체 시에만 필요하며 NVRAM DIMM 교체 시에는 필요하지 않습니다.
...
. 컨트롤러를 다시 제공합니다.
+
.. 정상 컨트롤러에서 교체된 컨트롤러의 스토리지를 반환합니다. `_storage failover giveback -ofnode impaired_node_name_`
+
