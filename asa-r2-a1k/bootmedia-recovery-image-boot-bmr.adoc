...
summary: ASA A1K 시스템의 교체 부팅 미디어에서 ONTAP 이미지를 복원합니다. 이 프로세스에는 Loader 프롬프트에서 부팅 복구 프로세스를 시작하고 복구 프로세스가 완료되고 노드가 재부팅될 때까지 복구 프로세스를 모니터링하는 작업이 포함됩니다. 
---
= 자동 부팅 복구 프로세스를 사용하여 ASA A1K 시스템의 부팅 이미지를 복원합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA A1K 시스템에 새 부팅 미디어 장치를 설치한 후 자동 부팅 미디어 복구 프로세스를 시작하여 파트너 노드에서 구성을 복원할 수 있습니다.
