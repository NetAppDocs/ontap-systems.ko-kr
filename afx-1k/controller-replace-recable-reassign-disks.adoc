...
summary: AFX 1K 스토리지 시스템이 정상적인 작동을 재개할 수 있도록 스토리지 리소스의 제어권을 교체 컨트롤러로 반환하세요. 
---
= AFX 1K 스토리지 시스템에서 컨트롤러를 반환하여 정상 작동을 재개합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX 1K 스토리지 시스템이 정상적인 작동을 재개할 수 있도록 스토리지 리소스의 제어권을 교체 컨트롤러로 반환하세요.  반환 절차는 시스템에서 사용하는 암호화 유형(암호화 없음 또는 Onboard Key Manager(OKM) 암호화)에 따라 다릅니다.
