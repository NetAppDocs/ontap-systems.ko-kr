...
summary: 정확한 시간 동기화에 의존하는 서비스 및 애플리케이션이 계속 작동하도록 하려면 AFX 1K 스토리지 시스템의 실시간 클록 배터리를 교체하십시오. 
---
= AFX 1K 스토리지 시스템의 실시간 클록 배터리 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
정확한 시간 동기화에 의존하는 서비스와 애플리케이션이 계속 작동할 수 있도록 AFX 1K 스토리지 시스템의 실시간 클록(RTC) 배터리(일반적으로 코인 셀 배터리라고 함)를 교체하세요.
...
고장난 RTC 배터리를 제거하고 교체용 배터리를 설치하세요.
...
NOTE: 홀더에서 배터리를 제거할 때 배터리의 극성을 주의하세요.  배터리에는 플러스 기호가 표시되어 있으며 홀더에 올바르게 넣어야 합니다.  홀더 근처의 더하기 기호는 올바른 방향을 나타냅니다.
...
NOTE: RTC 배터리를 교체하고 컨트롤러를 삽입한 후 첫 번째 BIOS 재설정을 실행하면 다음과 같은 오류 메시지가 표시됩니다.
`RTC date/time error. Reset date/time to default`
`RTC power failure error` 이러한 메시지는 예상된 것이며, 계속해서 절차를 진행할 수 있습니다.

. 건강한 컨트롤러의 날짜와 시간을 확인하세요. `cluster date show` 명령.
+

NOTE: 시스템이 부팅 메뉴에서 중지되면 옵션을 선택하고 메시지가 표시되면 respect_y_를 선택한  `Reboot node` 다음 _Ctrl-C_를 눌러 LOADER로 부팅합니다

+
.. 대상 컨트롤러의 LOADER 프롬프트에서 시간과 날짜를 확인하십시오. `show date` 명령.
...
. 반환 보고서가 완료된 후 5분 정도 기다린 후 장애 조치 및 반환 상태를 확인하세요.
+
