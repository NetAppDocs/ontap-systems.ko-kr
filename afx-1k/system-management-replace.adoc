...
summary: AFX 1K 스토리지 시스템의 시스템 관리 모듈에 결함이 생기거나 펌웨어가 손상된 경우 교체하세요.  교체 프로세스에는 컨트롤러 종료, 오류가 발생한 시스템 관리 모듈 교체, 컨트롤러 재부팅, 라이선스 키 업데이트, 오류가 발생한 부품을 NetApp 으로 반환하는 작업이 포함됩니다. 
---
= AFX 1K 스토리지 시스템의 시스템 관리 모듈 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX 1K 스토리지 시스템의 시스템 관리 모듈에 결함이 생기거나 펌웨어가 손상된 경우 교체하세요.  교체 프로세스에는 컨트롤러 종료, 오류가 발생한 시스템 관리 모듈 교체, 컨트롤러 재부팅, 라이선스 키 업데이트, 오류가 발생한 부품을 NetApp 으로 반환하는 작업이 포함됩니다.

컨트롤러 후면 8번 슬롯 왼쪽에 위치한 시스템 관리 모듈에는 시스템 관리를 위한 온보드 구성 요소와 외부 관리용 포트가 포함되어 있습니다. 손상된 시스템 관리 모듈을 교체하거나 부팅 미디어를 교체하려면 대상 컨트롤러를 종료하고 전원을 꺼야 합니다.
