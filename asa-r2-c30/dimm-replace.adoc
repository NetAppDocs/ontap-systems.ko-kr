...
summary: 과도한 수정 가능 또는 수정 불가능 메모리 오류가 감지된 경우 ASA C30 스토리지 시스템에서 DIMM을 교체하세요. 이러한 오류는 스토리지 시스템이 ONTAP를 부팅하지 못할 수 있습니다. 교체 프로세스에는 손상된 컨트롤러를 종료하고, DIMM을 교체하고, 컨트롤러를 다시 설치한 다음, 장애가 발생한 부품을 NetApp로 반환하는 과정이 포함됩니다. 
---
= ASA C30 시스템의 DIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
과도한 수정 가능 또는 수정 불가능 메모리 오류가 감지된 경우 ASA C30 스토리지 시스템에서 DIMM을 교체하세요. 이러한 오류는 스토리지 시스템이 ONTAP를 부팅하지 못할 수 있습니다. 교체 프로세스에는 손상된 컨트롤러를 종료하고, DIMM을 교체하고, 컨트롤러를 다시 설치한 다음, 장애가 발생한 부품을 NetApp로 반환하는 과정이 포함됩니다.
