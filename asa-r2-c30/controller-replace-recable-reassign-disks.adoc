...
summary: 'ASA C30 스토리지 시스템이 정상 작동을 재개할 수 있도록 스토리지 리소스 제어권을 교체 컨트롤러로 반환하십시오. 반환 절차는 시스템에서 사용하는 암호화 유형(암호화 안 함, 온보드 키 관리자(OKM) 암호화 또는 외부 키 관리자(EKM) 암호화)에 따라 다릅니다.' 
---
= 컨트롤러 - ASA C30를 돌려줍니다
:allow-uri-read: 
:icons: font
