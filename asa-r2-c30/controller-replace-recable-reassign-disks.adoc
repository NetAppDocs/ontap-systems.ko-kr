...
summary: 'ASA C30 스토리지 시스템이 정상적으로 작동할 수 있도록 스토리지 리소스 제어권을 교체 컨트롤러로 반환하십시오. 제어권 반환 절차는 시스템에서 사용하는 암호화 유형(암호화 없음, Onboard Key Manager 암호화 또는 External Key Manager 암호화)에 따라 다릅니다.' 
---
= ASA C30 시스템의 컨트롤러를 반납하여 정상 작동을 재개하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA C30 스토리지 시스템이 정상적인 작동을 재개할 수 있도록 스토리지 리소스의 제어권을 교체 컨트롤러로 반환하세요. Giveback 절차는 암호화 없음, OKM(Onboard Key Manager) 암호화 또는 EKM(External Key Manager) 암호화 등 시스템에서 사용하는 암호화 유형에 따라 다릅니다.
