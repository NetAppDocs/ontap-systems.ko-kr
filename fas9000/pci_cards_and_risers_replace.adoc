...
== 3단계: PCIe 모듈을 교체한 후 컨트롤러를 재부팅합니다

[role="lead"]
PCIe 모듈을 교체한 후 컨트롤러 모듈을 재부팅해야 합니다.

.단계
. LOADER 프롬프트에 노드가 있으면 시스템 ID 불일치에 대한 프롬프트 경고가 나타나고 시스템 ID를 무시하라는 메시지가 나타나면 y를 사용하여 노드를 부팅합니다
