...
** 명령 출력에 <lno-DARE>가 표시되지 않고 시스템에서 ONTAP 9.5를 실행 중인 경우 로 이동합니다  1: Check NVE or NSE on systems running ONTAP 9.5 and earlier.
** 명령 출력에 <lno-DARE>가 표시되지 않고 시스템에서 ONTAP 9.6 이상을 실행 중인 경우 로 이동하십시오  2: Check NVE or NSE on systems running ONTAP 9.6 and later.
