...
summary: 팬이 고장나거나 효율적으로 작동하지 않는 경우 AFF C30 또는 AFF C60 스토리지 시스템의 팬 모듈을 교체하세요. 이는 시스템 냉각 및 전반적인 성능에 영향을 미칠 수 있습니다.  교체 프로세스에는 컨트롤러를 종료하고, 컨트롤러를 제거하고, 팬을 교체하고, 컨트롤러를 다시 설치하고, 오류가 발생한 부품을 NetApp 으로 반환하는 작업이 포함됩니다. 
---
= AFF C30 또는 AFF C60 스토리지 시스템의 팬 모듈 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
팬이 고장나거나 효율적으로 작동하지 않는 경우 AFF C30 또는 AFF C60 스토리지 시스템의 팬 모듈을 교체하세요. 이는 시스템 냉각 및 전반적인 성능에 영향을 미칠 수 있습니다.  교체 프로세스에는 컨트롤러를 종료하고, 컨트롤러를 제거하고, 팬을 교체하고, 컨트롤러를 다시 설치하고, 오류가 발생한 부품을 NetApp 으로 반환하는 작업이 포함됩니다.
