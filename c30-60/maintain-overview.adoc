...
[role="lead"]
AFF C30 또는 AFF C60 스토리지 시스템의 하드웨어를 관리하여 장기적인 안정성과 최적의 성능을 보장합니다. 고장 난 구성 요소 교체와 같은 정기 유지 관리 작업을 수행하면 가동 중지 시간 및 데이터 손실을 방지할 수 있습니다.

이 섹션의 절차는 AFF C30 또는 AFF C60 스토리지 시스템이 ONTAP 환경에서 스토리지 노드로 이미 구축되어 있다고 가정합니다.
