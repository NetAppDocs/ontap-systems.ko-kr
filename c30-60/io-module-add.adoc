...
summary: AFF C30 또는 AFF C60 스토리지 시스템에 I/O 모듈을 추가하여 네트워크 연결성을 향상시키고 시스템의 데이터 트래픽 처리 능력을 확장하십시오. 
---
= AFF C30 또는 AFF C60 스토리지 시스템에 I/O 모듈을 추가하여 네트워크 연결성을 향상시키십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C30 또는 AFF C60 스토리지 시스템에 I/O 모듈을 추가하여 네트워크 연결성을 향상시키고 시스템의 데이터 트래픽 처리 능력을 확장하십시오. 이 절차를 사용하여 계획된 업그레이드의 일환으로 운영 중인 컨트롤러에 I/O 모듈을 추가하거나 유지보수 윈도우 중에 손상된 컨트롤러에 I/O 모듈을 추가할 수 있습니다.

사용 가능한 슬롯이 있거나 모든 슬롯이 완전히 채워진 경우 AFF C30 및 AFF C60 스토리지 시스템에 입출력 모듈을 추가할 수 있습니다.

.이 작업에 대해
HA 쌍의 컨트롤러 중 어느 하나에서든 이 절차를 수행할 수 있으며, 컨트롤러가 현재 작동 중인지 또는 손상된 상태인지 여부는 중요하지 않습니다. I/O 모듈은 실행 중인 컨트롤러에 추가할 수 없으므로 모듈을 설치하기 전에 대상 컨트롤러를 종료해야 합니다. 이 절차가 진행되는 동안 HA 파트너가 스토리지 작업을 인계받아 클라이언트가 데이터에 계속 액세스할 수 있도록 합니다.

* 대상 컨트롤러가 *작동 중*인 경우(계획된 추가), 해당 콘텐츠에서 대상 컨트롤러를 "손상된" 컨트롤러로 간주하여 종료 단계를 따르십시오.
* 대상 컨트롤러가 *손상된* 경우 명시된 종료 단계를 따르십시오.
...
== 1단계: 타겟 컨트롤러 모듈을 종료합니다

대상 컨트롤러 모듈의 제어권을 인계받아 종료하면, I/O 모듈을 추가하는 동안에도 해당 HA 파트너 컨트롤러가 계속해서 데이터를 제공할 수 있습니다. 링크된 단계에서는 종료되는 컨트롤러를 "손상된" 컨트롤러라고 지칭하지만, 계획된 추가 작업을 위해 작동 중인 컨트롤러의 제어권을 인계받거나 이미 손상된 컨트롤러를 종료하는 경우에도 동일한 단계가 적용됩니다.
