...
summary: USB 드라이브에서 ONTAP 이미지를 부팅하고 파일 시스템을 복원한 다음 환경 변수를 확인해야 합니다. AFF A220 시스템은 수동 부팅 미디어 복구 절차만 지원합니다. 
---
= AFF A220 시스템에서 복구 이미지를 부팅합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A220 시스템의 USB 드라이브에서 ONTAP 복구 이미지를 부팅하여 수동 부팅 복구 프로세스를 통해 부팅 미디어를 복원하십시오.
