...
summary: 시스템에서 수정 가능한 오류 정정 코드 또는 수정 불가능한 ECC 오류가 증가하는 경우 ASA C400 컨트롤러 모듈의 DIMM을 교체하십시오. 결함이 있는 DIMM을 교체하지 않으면 시스템 패닉이 발생할 수 있습니다. 
---
= ASA C400 시스템의 메모리 오류를 해결하려면 DIMM을 교체하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA C400 스토리지 시스템에서 과도한 ECC(정정 가능한 오류 수정 코드) 오류가 발생하거나 수정 불가능한 ECC 오류가 발생하는 경우 DIMM을 교체하십시오. 이러한 오류는 일반적으로 단일 DIMM 고장으로 인해 발생합니다.
