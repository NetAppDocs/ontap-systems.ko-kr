...
summary: 시스템에서 플래시 메모리의 수명이 거의 다 되었거나 식별된 NVDIMM이 전반적으로 정상적이지 않다고 표시되면 컨트롤러 모듈의 NVDIMM을 교체해야 합니다. 그렇지 않으면 시스템 패닉이 발생합니다. 
---
= ASA C400 시스템의 NVDIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA C400 시스템에서 스토리지 시스템에 수정 가능한 ECC 오류가 과도하게 발생하거나 수정 불가능한 ECC 오류가 발생하는 경우 NVDIMM을 교체하십시오. 이러한 오류는 일반적으로 단일 NVDIMM 오류로 인해 발생합니다.
