...
[role="lead"]
부트 이미지 자동 복구는 시스템이 적절한 부트 메뉴 옵션을 자동으로 식별하고 선택하는 과정을 포함합니다. AFF A400 스토리지 시스템의 교체 부트 미디어에 ONTAP 다시 설치하려면 파트너 노드의 부트 이미지를 사용합니다.
