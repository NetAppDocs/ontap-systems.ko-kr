...
summary: AFF A400 스토리지 시스템의 부트 미디어 교체를 시작하려면 교체 요구 사항을 검토하고, 암호화 상태를 확인하고, 컨트롤러를 종료하고, 부트 미디어를 교체하고, 복구 이미지를 부팅하고, 암호화를 복원하고, 시스템 기능을 검증하세요. 
---
= AFF A400 부팅 미디어 교체 워크플로(수동 부팅 복구 프로세스 사용)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A400 수동 부팅 복구 프로세스를 사용하여 AFF A400 시스템의 부팅 미디어를 교체하는 방법을 알아보세요. 복구를 수행하고 시스템을 정상 작동 상태로 복원하려면 NetApp 지원 사이트에서 적절한 ONTAP 복구 이미지를 다운로드하여 USB 드라이브에 복사해야 합니다.
