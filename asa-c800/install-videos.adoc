---
permalink: asa-c800/install-videos.html 
sidebar: sidebar 
keywords: asa c800, hardware installation and cabling video, perform end-to-end software configuration video 
summary: 시스템 랙에 설치하고 케이블을 연결하는 방법을 보여주는 비디오 1개와 System Manager Guided Setup을 사용하여 초기 시스템 구성을 수행하는 예를 보여 주는 비디오 1개가 있습니다. 
---
= 동영상 안내를 통해 ASA C800 스토리지 시스템을 설치하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
시스템 랙에 설치하고 케이블을 연결하는 방법을 보여주는 비디오 1개와 System Manager Guided Setup을 사용하여 초기 시스템 구성을 수행하는 예를 보여 주는 비디오 1개가 있습니다.



[[video-one-of-two-hardware-installation-and-cabling]]
== 비디오 2: 하드웨어 설치 및 케이블 연결

다음 비디오에서는 새 시스템의 설치 및 케이블 연결 방법을 보여줍니다.

."애니메이션 - ASA C800의 설치 및 설정
video::2a61ed74-a0ce-46c3-86d2-ab4b013c0030[panopto]


[[video-two-of-two-perform-end-to-end-software-configuration]]
== 비디오 2: 종단 간 소프트웨어 구성을 수행합니다

다음 비디오에서는 ONTAP 9.2 이상을 실행하는 시스템의 엔드 투 엔드 소프트웨어 구성을 보여줍니다.

video::Q6orVMyj94A?[youtube]