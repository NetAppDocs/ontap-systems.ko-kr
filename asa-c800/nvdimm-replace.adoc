...
summary: 시스템에서 플래시 수명이 거의 다 되었거나 식별된 NVDIMM이 정상 상태가 아니라고 등록하면 컨트롤러 모듈의 NVDIMM을 교체해야 합니다. 그렇지 않으면 시스템 패닉이 발생합니다. 
---
= ASA C800 시스템의 NVDIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA C800 시스템에서 플래시 메모리의 수명이 거의 다 되었거나 식별된 NVDIMM이 전반적으로 정상적이지 않다고 표시되면 NVDIMM을 교체하십시오. 그렇지 않으면 시스템 오류가 발생합니다.

.시작하기 전에
* NetApp 에서 받은 교체용 NVDIMM이 ASA C800 시스템과 호환되는지 확인하십시오.
