...
[role="lead"]
ASA C800 시스템의 팬이 고장 나거나 효율적으로 작동하지 않으면 시스템 냉각 및 전반적인 성능에 영향을 미칠 수 있으므로 팬 모듈을 교체하십시오.  교체 과정은 고장난 컨트롤러를 종료하고, 컨트롤러를 제거하고, 팬 모듈을 교체하고, 컨트롤러를 다시 설치한 다음, 고장난 부품을 NetApp 으로 반환하는 과정을 포함합니다.
