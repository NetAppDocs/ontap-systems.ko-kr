...
summary: 'NVMe SSD Flash Cache 모듈 캐리어는 각 캐싱 모듈에 통합된 단일 SSD 캐싱 드라이브를 갖춘 하나 또는 2개의 Flash Cache 모듈(캐싱 모듈)을 포함합니다. 캐시 모듈은 핫 스왑할 수 있지만 Flash Cache 모듈 캐리어는 핫 스왑이 가능하지 않습니다.' 
---
...
[role="lead"]
NVMe SSD Flash Cache 모듈 캐리어는 각 캐싱 모듈에 통합된 단일 SSD Flash Cache 드라이브를 갖춘 하나 또는 2개의 Flash Cache 모듈(캐싱 모듈)을 포함합니다.

FAS70은 2TB 캐싱 모듈을 지원하며 FAS90은 4TB 캐싱 모듈을 지원합니다. 서로 다른 용량의 캐싱 모듈을 Flash Cache 모듈 캐리어에서 혼합할 수 없습니다.

교체해야 하는 항목에 따라 다음 절차 중 하나를 수행할 수 있습니다. 전체 Flash Cache 모듈 캐리어 또는 캐싱 모듈.
