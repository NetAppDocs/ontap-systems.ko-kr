...
summary: 'FAS70 또는 FAS90 시스템의 실시간 클록 RTC 배터리(일반적으로 코인형 배터리라고 함)를 교체하여 정확한 시간 동기화에 의존하는 서비스 및 애플리케이션이 계속 작동하도록 하십시오.' 
---
= 실시간 클럭 배터리를 교체합니다. - FAS70 및 FAS90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS70 또는 FAS90 시스템에서 일반적으로 코인 셀 배터리라고 하는 실시간 클럭(RTC) 배터리를 교체하여 정확한 시간 동기화에 의존하는 서비스 및 애플리케이션이 계속 작동하도록 합니다.
