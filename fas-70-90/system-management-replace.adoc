...
summary: FAS70 또는 FAS90 시스템에 결함이 있거나 펌웨어가 손상된 경우 시스템 관리 모듈을 교체하십시오. 교체 프로세스에는 컨트롤러 종료, 장애가 발생한 시스템 관리 모듈 교체, 컨트롤러 재부팅, 라이센스 키 업데이트, 실패한 부품 NetApp 반환 등이 포함됩니다. 
---
= 시스템 관리 모듈 FAS70 및 FAS90을 교체합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS70 또는 FAS90 시스템에 결함이 있거나 펌웨어가 손상된 경우 시스템 관리 모듈을 교체하십시오. 교체 프로세스에는 컨트롤러 종료, 장애가 발생한 시스템 관리 모듈 교체, 컨트롤러 재부팅, 라이센스 키 업데이트, 실패한 부품 NetApp 반환 등이 포함됩니다.
