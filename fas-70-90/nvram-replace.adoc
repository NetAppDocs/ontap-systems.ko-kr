...
summary: 비휘발성 메모리에 오류가 발생하거나 업그레이드가 필요한 경우 FAS70 또는 FAS90 시스템의 NVRAM를 교체합니다. 
---
...
[role="lead"]
비휘발성 메모리에 오류가 발생하거나 업그레이드가 필요한 경우 FAS70 또는 FAS90 시스템의 NVRAM를 교체합니다. 교체 프로세스에는 손상된 컨트롤러를 종료하고, NVRAM 모듈 또는 NVRAM DIMM을 교체하고, 디스크를 재할당하고, 장애가 발생한 부품을 NetApp에 반환하는 작업이 포함됩니다.
