...
. 정상 작동이 확인된 노드에서 명령을 사용하여 자동 반환을 비활성화한 경우 `storage failover modify -node local -auto-giveback true` 복원합니다.
...
. 파트너 컨트롤러에서 컨트롤러를 반환하십시오. '스토리지 페일오버 반환 - ofnode target_node_name'입니다
