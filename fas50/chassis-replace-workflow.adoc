...
summary: FAS50 스토리지 시스템 섀시를 교체하려면 다음 워크플로 단계를 따르세요. 
---
= 섀시 교체 워크플로우 - FAS50
:allow-uri-read: 
:icons: font
...
[role="lead"]
FAS50 스토리지 시스템 섀시를 교체하려면 다음 워크플로 단계를 따르세요.
...
[role="quick-margin-para"]
섀시 교체에는 드라이브 보호물, 캐시 모듈, 컨트롤러(전원 공급 장치 포함) 및 베젤을 손상된 섀시에서 새 섀시로 이동하고 손상된 섀시를 손상된 섀시와 동일한 모델의 새 섀시로 바꾸는 작업이 포함됩니다.
