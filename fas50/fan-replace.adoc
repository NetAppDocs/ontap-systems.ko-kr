...
summary: 팬이 고장나거나 효율적으로 작동하지 않으면 시스템 냉각 및 전체 성능에 영향을 미칠 수 있으므로 FAS50 시스템에서 팬 모듈을 교체하십시오. 교체 프로세스에는 컨트롤러 종료, 컨트롤러 제거, 팬 교체, 컨트롤러 재설치 및 오류가 발생한 부품을 NetApp에 반환하는 작업이 포함됩니다. 
---
= 팬 모듈 교체 - FAS50
:allow-uri-read: 
:icons: font
...
[role="lead"]
팬이 고장나거나 효율적으로 작동하지 않으면 시스템 냉각 및 전체 성능에 영향을 미칠 수 있으므로 FAS50 시스템에서 팬 모듈을 교체하십시오. 교체 프로세스에는 컨트롤러 종료, 컨트롤러 제거, 팬 교체, 컨트롤러 재설치 및 오류가 발생한 부품을 NetApp에 반환하는 작업이 포함됩니다.
