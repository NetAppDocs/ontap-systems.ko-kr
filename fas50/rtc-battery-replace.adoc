...
summary: FAS50 스토리지 시스템에서 코인 셀 배터리로 일반적으로 알려진 실시간 클록 배터리를 교체하여 정확한 시간 동기화에 의존하는 서비스 및 애플리케이션이 계속 작동하도록 하십시오. 
---
= 실시간 시계 배터리 교체 - FAS50
:allow-uri-read: 
:icons: font
...
[role="lead"]
정확한 시간 동기화에 의존하는 서비스와 애플리케이션이 계속 작동할 수 있도록 FAS50 스토리지 시스템의 실시간 클록(RTC) 배터리(일반적으로 코인 셀 배터리라고 함)를 교체하세요.
