...
sidebar: sidebar 
keywords: aff a20, aff a30, aff a50, add IO module 
summary: AFF A20, AFF A30 또는 AFF A50 시스템에 I/O 모듈을 추가하여 네트워크 연결을 개선하고 시스템의 데이터 트래픽 처리 능력을 확장하십시오. 
---
= AFF A20, AFF A30 또는 AFF A50 시스템에 I/O 모듈을 추가하여 네트워크 연결성을 향상시킵니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
추가 프로세스에는 한 컨트롤러를 종료하고, 새 I/O 모듈을 추가하고, 컨트롤러를 재부팅하고, 다른 컨트롤러를 종료하고, 새 I/O 모듈을 추가하고, 컨트롤러를 재부팅하는 작업이 포함됩니다.

.이 작업에 대해
* 사용 가능한 슬롯이 있거나 모든 슬롯이 가득 찼을 때 스토리지 시스템에 I/O 모듈을 추가할 수 있습니다.
