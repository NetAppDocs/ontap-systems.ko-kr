...
[role="lead"]
필요한 경우 NetApp 스토리지 암호화 구성을 복원하고, 교체 컨트롤러에 대한 라이센스를 설치하고, 논리 인터페이스(LIF)가 홈 포트에 보고하는지 확인하고, 클러스터 상태 점검을 수행하고, 교체 컨트롤러의 일련 번호를 등록한 다음, 장애가 발생한 부품을 NetApp에 반환하여 AFF A20, AFF A30 또는 AFF A50 컨트롤러 교체 절차의 최종 단계를 완료합니다.
