...
[role="lead"]
장기적인 안정성과 최적의 성능을 보장하려면 FAS2820 스토리지 시스템의 하드웨어를 유지 관리하십시오. 고장난 부품 교체와 같은 정기적인 유지 관리 작업을 수행하면 가동 중지 및 데이터 손실을 방지하는 데 도움이 됩니다.

유지 관리 절차에서는 FAS2820 스토리지 시스템이 이미 ONTAP 환경에서 스토리지 노드로 배포되었다고 가정합니다.



== 시스템 구성 요소

FAS2820 스토리지 시스템의 경우 다음 구성 요소에서 유지보수 절차를 수행할 수 있습니다.
...
배터리는 컨트롤러에 포함되어 있으며 AC 전원에 장애가 발생할 경우 캐싱된 데이터를 보존합니다.
 ...
 a| 
link:pci-cards-and-risers-replace.html["메자닌 카드"]
 a| 
메자닌 카드는 마더보드의 특수 슬롯에 삽입하도록 설계된 확장 카드로, 카드 I/O 카드를 수용합니다.



 a| 
link:power-supply-swap-out.html["전원 공급 장치"]
 a| 
