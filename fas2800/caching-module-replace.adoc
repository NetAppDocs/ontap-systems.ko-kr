...
summary: 시스템에서 컨트롤러 모듈의 캐싱 모듈이 오프라인 상태가 되었다는 단일 AutoSupport ASUP 메시지가 등록되면 해당 모듈을 교체해야 합니다. 장애가 발생한 모듈을 교체하지 않으면 스토리지 시스템의 성능 저하가 발생합니다. 
---
= 캐싱 모듈 교체 - FAS2820
:allow-uri-read: 
:icons: font
...
+
image::../media/drw_2850_open_controller_module_cover_IEOPS-695.svg[컨트롤러 모듈을 엽니다]
...
+
image::../media/drw_2850_replace_caching module_IEOPS-697.svg[캐싱을 교체하세요]
