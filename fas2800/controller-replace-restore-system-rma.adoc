...
summary: NetApp Storage Encryption 또는 Volume Encryption 구성을 복원하고 교체 컨트롤러에 대한 라이센스를 설치한 다음 키트에 포함된 RMA 지침에 따라 고장난 부품을 NetApp으로 반환하여 시스템을 완전히 복구하십시오. 
---
= 전체 시스템 복원 - FAS2820
:allow-uri-read: 
:icons: font
...
[role="lead"]
NetApp 스토리지 암호화 또는 볼륨 암호화 구성(필요한 경우)을 복원하고 교체 컨트롤러에 대한 라이센스를 설치한 다음 키트와 함께 제공된 RMA 지침에 따라 오류가 발생한 부품을 NetApp로 반환하여 시스템을 전체 운영으로 복원합니다.
