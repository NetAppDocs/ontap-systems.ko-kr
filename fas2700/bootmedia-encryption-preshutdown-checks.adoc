...
sidebar: sidebar 
keywords: fas2700, aff a220, shutdown, checks, onboard, encryption, keys, impaired controller, disable, automatic, feedback, manual, manual boot recovery 
summary: FAS2700 시스템에서 문제가 발생한 컨트롤러를 종료하기 전에 암호화 키 지원 여부와 상태를 확인하십시오. FAS2700 시스템은 수동 부팅 미디어 복구 절차만 지원합니다. 자동 부팅 미디어 복구는 지원되지 않습니다. 
---
= FAS2700 암호화 키 지원 및 상태 확인
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS2700 시스템에서 문제가 발생한 컨트롤러를 종료하기 전에 암호화 키 지원 여부와 상태를 확인하십시오. 이 절차에는 NetApp 볼륨 암호화(NVE)와의 ONTAP 버전 호환성 확인, 키 관리자 구성 검증, 부팅 미디어 복구 중 데이터 보안을 보장하기 위한 암호화 정보 백업이 포함됩니다.

FAS2700 시스템은 수동 부팅 미디어 복구 절차만 지원합니다. 자동 부팅 미디어 복구는 지원되지 않습니다.
