...
sidebar: sidebar 
keywords: fas2700, aff a220, replace, nvmem, battery, shut, down, impaired, controller, node, system, diagnostics, aggregates, metrocluster, configuration, reinstall 
summary: 시스템에서 NVMEM 배터리를 교체하려면 컨트롤러 모듈을 시스템에서 분리하고, 배터리를 개봉하고, 배터리를 교체하고, 컨트롤러 모듈을 닫고 교체해야 합니다. 
---
= NVMEM 배터리 - AFF A220 및 FAS2700을 교체합니다
:icons: font
:imagesdir: ../media/
...
** HA 쌍의 경우 정상 노드에서 스토리지 페일오버 테이크오버인 ofnode_impaired_node_name _ 을(를) 인수합니다
+
손상된 노드에 반환 대기 중... 이 표시되면 Ctrl-C를 누른 다음 "y"로 응답합니다.
...
|===
. 시스템에 섀시에 하나의 컨트롤러 모듈만 있는 경우 전원 공급 장치를 끈 다음 손상된 노드의 전원 코드를 전원에서 분리합니다.
