...
AFF C190 시스템을 설치하려면 계정을 만들고 시스템을 등록해야 합니다. 또한 시스템에 맞는 케이블 수와 유형을 확인하고 특정 네트워크 정보를 수집해야 합니다.

에 대한 액세스 권한이 있어야 합니다 link:https://hwu.netapp.com["NetApp Hardware Universe를 참조하십시오"^] (HWU) - 사이트 요구사항에 대한 정보 및 구성된 시스템에 대한 추가 정보 에 대한 액세스 권한을 가질 수도 있습니다  이 시스템에 대한 자세한 내용은 를 참조하십시오.
...
=== 옵션 1: 스위치가 없는 2노드 클러스터 및 유니파이드 구성을 연결합니다

컨트롤러 모듈의 UTA2 포트 및 관리 포트는 스위치에 연결됩니다. 클러스터 인터커넥트 포트는 두 컨트롤러 모듈에 케이블로 연결됩니다.
...
.단계
. 그림 또는 단계별 지침을 사용하여 컨트롤러와 스위치 간 케이블 연결을 완료합니다.
+
...
image:../media/oie_legend_icon_2_o.png[""]
 a| 
다음 케이블 유형 중 하나를 사용하여 e0c/0c 및 e0d/0d * 또는 * e0e/0e 및 e0f/0f 데이터 포트를 호스트 네트워크에 연결합니다.
...
=== 옵션 2: 케이블 스위치 클러스터, 유니파이드 구성

컨트롤러 모듈의 UTA2 포트 및 관리 포트는 스위치에 연결됩니다. 클러스터 인터커넥트 포트는 클러스터 인터커넥트 스위치에 케이블로 연결됩니다.
...
.단계
. 그림 또는 단계별 지침을 사용하여 컨트롤러와 스위치 간의 케이블 연결을 완료합니다.
+
...
=== 옵션 3: 스위치가 없는 2노드 클러스터, 이더넷 구성을 케이블로 연결합니다

컨트롤러 모듈의 RJ45 포트 및 관리 포트는 스위치에 연결됩니다. 클러스터 인터커넥트 포트는 두 컨트롤러 모듈에 케이블로 연결됩니다.
...
image:../media/oie_legend_icon_1_lg.png[""]
 a| 
클러스터 인터커넥트 케이블을 사용하여 클러스터 인터커넥트 포트를 서로 케이블로 연결합니다 image:../media/oie_cable_sfp_gbe_copper.png[""]:
...
image:../media/oie_legend_icon_3_lp.png[""]
 a| 
RJ45 케이블을 사용하여 e0M 포트를 관리 네트워크 스위치에 연결합니다 image:../media/oie_cable_rj45.png[""].
...
=== 옵션 4: 스위치 클러스터 케이블 연결, 이더넷 구성

컨트롤러 모듈의 RJ45 포트 및 관리 포트는 스위치에 연결됩니다. 클러스터 인터커넥트 포트는 클러스터 인터커넥트 스위치에 케이블로 연결됩니다.
...
. 애니메이션(link:https://netapp.hosted.panopto.com/Panopto/Pages/embed.aspx?id=d61f983e-f911-4b76-8b3a-ab1b0066909b["관리 스위치에 랩톱 연결"^])를 클릭하여 노트북을 관리 스위치에 연결합니다.
