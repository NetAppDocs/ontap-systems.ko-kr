...
summary: 섀시를 교체해야 하는 경우 ASA A70 또는 ASA A90 스토리지 시스템에서 컨트롤러를 종료합니다. 데이터 손실을 방지하고 시스템 안정성을 보장하려면 컨트롤러를 안전하게 종료하는 것이 중요합니다. 
---
= ASA A70 및 ASA A90 컨트롤러를 종료합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
섀시를 교체해야 하는 경우 ASA A70 또는 ASA A90 스토리지 시스템에서 컨트롤러를 종료합니다. 데이터 손실을 방지하고 시스템 안정성을 보장하려면 컨트롤러를 안전하게 종료하는 것이 중요합니다.
