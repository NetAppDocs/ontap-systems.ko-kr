---
permalink: a1k/controller-replace-restore-system-rma.html 
sidebar: sidebar 
keywords: aff a1k, how to restore NetApp Storage Encryption, how to complete the replacement process 
summary: AFF A1K 시스템의 컨트롤러 교체를 완료하려면 NetApp Storage Encryption 구성을 복원하고, 논리 인터페이스가 홈 포트로 보고하는지 확인하고, 클러스터 상태 점검을 수행한 다음, 고장난 부품을 NetApp에 반환하십시오. 
---
= AFF A1K 시스템의 컨트롤러 교체를 완료합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
AFF A1K 시스템의 컨트롤러 교체를 완료하려면 먼저 NetApp 스토리지 암호화 구성을 복원하십시오(필요한 경우). 그런 다음 논리 인터페이스(LIF)가 홈 포트에 보고하는지 확인하고 클러스터 상태 점검을 수행합니다. 마지막으로, 문제가 발생한 부분을 NetApp으로 반환하십시오.



[[step-1-verify-lifs-and-check-cluster-health]]
== 1단계: LIF 확인 및 클러스터 상태 확인

교체 노드를 서비스로 반환하기 전에 논리 인터페이스가 홈 포트에 있는지 확인하고 클러스터 상태를 확인한 다음 자동 반환을 재설정합니다.

.단계
. 논리적 인터페이스가 홈 서버와 포트에 보고하는지 확인하세요.
+
`network interface show -is-home false`

+
논리적 인터페이스가 거짓으로 나열된 경우 해당 인터페이스를 홈 포트로 되돌립니다.

+
`network interface revert -vserver * -lif *`

. 클러스터의 상태를 확인하세요.  를 참조하십시오 https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/How_to_perform_a_cluster_health_check_with_a_script_in_ONTAP["ONTAP에서 스크립트를 사용하여 클러스터 상태 점검을 수행하는 방법"^] KB 문서.
. 자동 반환이 비활성화된 경우 다시 활성화하십시오.
+
`storage failover modify -node local -auto-giveback true`





[[step-2-return-the-failed-part-to-netapp]]
== 2단계: 장애가 발생한 부품을 NetApp로 되돌립니다

키트와 함께 제공된 RMA 지침에 설명된 대로 오류가 발생한 부품을 NetApp에 반환합니다.  https://mysupport.netapp.com/site/info/rma["부품 반환 및 교체"]자세한 내용은 페이지를 참조하십시오.
