...
summary: 시스템에서 수정 가능한 오류 정정 코드 또는 수정 불가능한 ECC 오류가 증가하는 경우 AFF A300 컨트롤러 모듈의 DIMM을 교체하십시오. 결함이 있는 DIMM을 교체하지 않으면 시스템 패닉이 발생할 수 있습니다. 
---
= AFF A300 시스템의 DIMM 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A300 시스템에서 스토리지 시스템에 과도한 수정 가능한 ECC 오류 또는 수정 불가능한 ECC 오류가 발생하는 경우(일반적으로 단일 DIMM 오류로 인해 발생) 해당 DIMM을 교체하십시오.
