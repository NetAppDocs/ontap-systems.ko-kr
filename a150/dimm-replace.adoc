...
summary: 스토리지 시스템에서 상태 모니터 경고에 따른 과도한 CECC 오류 또는 수정 불가능한 ECC 오류와 같은 오류가 발생하는 경우 컨트롤러의 DIMM을 교체해야 합니다. 이러한 오류는 일반적으로 단일 DIMM 장애로 인해 스토리지 시스템이 ONTAP를 부팅하지 못하게 됩니다. 
---
= AFF A150 시스템의 메모리 오류를 해결하려면 DIMM을 교체하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A150 시스템에서 스토리지 시스템에 과도한 수정 가능(CECC) 오류 또는 수정 불가능 메모리 오류가 발생하면 DIMM을 교체하십시오. 이러한 오류로 인해 스토리지 시스템이 ONTAP를 부팅하지 못할 수 있습니다.
