...
[role="lead"]
AFF A150 시스템의 컨트롤러 교체를 완료하려면 NetApp Storage Encryption 구성을 복원하고(필요한 경우), 논리 인터페이스(LIF)가 홈 포트에 보고하는지 확인하고, 클러스터 상태 점검을 수행한 다음, 고장난 부품을 NetApp에 반환하십시오.
