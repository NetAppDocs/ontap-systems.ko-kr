...
[role="lead"]
AFF A150 시스템의 NVMEM 배터리는 정전 시 중요한 시스템 데이터를 보존하므로, 배터리 충전량이 감소하거나 고장이 발생하면 교체하십시오. 교체 과정은 컨트롤러 모듈을 분리하고, 배터리를 교체한 후, 컨트롤러 모듈을 다시 설치하는 방식으로 진행됩니다.
