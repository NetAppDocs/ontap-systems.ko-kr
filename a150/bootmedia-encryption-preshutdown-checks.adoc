...
---
= AFF A150 시스템의 부팅 미디어에서 암호화 지원 여부를 확인하십시오
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A150 스토리지 시스템에서 데이터 보안을 보장하려면 부팅 미디어에서 암호화 키 지원 및 상태를 확인해야 합니다. ONTAP 버전이 NetApp Volume Encryption(NVE)을 지원하는지 확인하고, 컨트롤러를 종료하기 전에 키 관리자가 활성화되어 있는지 확인하십시오.
